...
export default class Header extends React.Component {
  ...
  render() {
    return <nav className={"navbar navbar-default" + " " + this.props.styleName || ''}
                style={{"zIndex": 99}}>
      <div className={"container-fluid navBackground" + " " + this.props.styleName || ''}>
        <div className="navbar-header">
          <button type="button"
                  className={"navbar-toggle collapsed navCollapsedButton" + " " + this.props.styleName || ''}
                  data-toggle="collapse" data-target="#bs-example-navbar-collapse-1"
                  aria-expanded="false">
            <span className="sr-only">Toggle navigation</span>
            <span className="icon-bar navCollapsedButtonItem"/>
        ...
        <div className="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
          <ul className="nav navbar-nav navbar-right">
            ...
            <li className="rightAligned"><a className="navButton" href="/logout"><span
              className="glyphicon glyphicon-log-out navIcon"/>&nbsp;&nbsp;Logout
              as {localStorage.ccUsername}</a></li>
          </ul>
        </div>
      </div>
    </nav>
